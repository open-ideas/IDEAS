within IDEAS.Fluid.Storage.Validation;
 ...
 constant Integer nSeg = 7 "Number of segments in tank";
  ...
  IDEAS.Fluid.Sources.Boundary_pT sou_1(
    ...
    nPorts=2)             annotation (Placement(transformation(extent={{-100,-20},
            {-80,0}})));
  IDEAS.Fluid.Sources.MassFlowSource_T sin_1(
    redeclare package Medium = Medium,
    T=273.15 + 20,
    m_flow=-0.028,
    use_m_flow_in=true,
    nPorts=1) annotation (Placement(transformation(extent={{78,-2},{58,18}})));
  IDEAS.Fluid.Storage.StratifiedEnhanced tanEnh(
    redeclare package Medium = Medium,
    hTan=3,
    dIns=0.3,
    VTan=0.1,
  ...
  IDEAS.Fluid.Sources.MassFlowSource_T sin_2(
    redeclare package Medium = Medium,
    T=273.15 + 20,
    m_flow=-0.028,
    use_m_flow_in=true,
  ...
  IDEAS.Fluid.Sensors.EnthalpyFlowRate HIn_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{-60,-38},{-44,-22}})));
  IDEAS.Fluid.Sensors.EnthalpyFlowRate HOut_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{22,-38},{38,-22}})));
  IDEAS.Fluid.Sensors.EnthalpyFlowRate HInEnh_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{-60,0},{-44,16}})));
  IDEAS.Fluid.Sensors.EnthalpyFlowRate HOutEnh_flow(redeclare package
...
equation
  ...
  connect(pulse.y, sin_2.m_flow_in) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  annotation (                     __Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Fluid/Storage/Validation/StratifiedLoadingUnloading.mos"
        "Simulate and plot"),
