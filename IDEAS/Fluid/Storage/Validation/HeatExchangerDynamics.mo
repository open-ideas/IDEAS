...
model HeatExchangerDynamics
  ...
  IDEAS.Fluid.Sources.Boundary_pT watInTan(
  ...
  IDEAS.Fluid.Sources.MassFlowSource_T mHex_flow1(
    redeclare package Medium = Medium,
    use_m_flow_in=true,
    T=273.15 + 60,
  ...
  IDEAS.Fluid.Sources.MassFlowSource_T mHex_flow2(
    redeclare package Medium = Medium,
    use_m_flow_in=true,
    T=273.15 + 60,
    nPorts=1) "Mass flow rate through the heat exchanger"
    annotation (Placement(transformation(extent={{-60,-20},{-40,0}})));
  IDEAS.Fluid.Sensors.TemperatureTwoPort senTanDyn(
    redeclare package Medium = Medium,
    allowFlowReversal=false,
    m_flow_nominal=m_flow_nominal,
    tau=0) "Temperature sensor at tank outlet"
    annotation (Placement(transformation(extent={{10,0},{-10,20}})));
  IDEAS.Fluid.Sensors.TemperatureTwoPort senTanSte(
    redeclare package Medium = Medium,
    allowFlowReversal=false,
    m_flow_nominal=m_flow_nominal,
    tau=0) "Temperature sensor at tank outlet"
    annotation (Placement(transformation(extent={{10,-50},{-10,-30}})));
  IDEAS.Fluid.Sources.MassFlowSource_T mWatTanDyn_flow(
    redeclare package Medium = Medium,
    nPorts=1) "Mass flow rate through the tank"
    annotation (Placement(transformation(extent={{88,20},{68,40}})));
  IDEAS.Fluid.Sources.MassFlowSource_T mWatTanSte_flow(
    redeclare package Medium = Medium,
    nPorts=1) "Mass flow rate through the tank"
...
equation
  ...
  annotation (
  __Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Fluid/Storage/Validation/HeatExchangerDynamics.mos"
        "Simulate and plot"),
