...
model ElectricalPV
  ...
  Modelica.Blocks.Math.Add Gtil "Total irradiation on tilted surface"
  ...
  Modelica.Blocks.Sources.RealExpression solarPower(y=n*P_STC*G/G_STC*(1+beta*T_diff)*(1-eleLosFac))
    "Power generated by the photovoltaic installation" annotation (Placement(transformation(extent={{11,-52},
            {-9,-32}})));
...
public
  outer Modelica.Blocks.Sources.CombiTimeTable meaDat(
    tableOnFile=true,
    tableName="data",
    fileName=Modelica.Utilities.Files.loadResource("modelica://IDEAS/Resources/Data/Fluid/PVTCollectors/Validation/PVT_UI/PVT_UI_Typ1_measurements.txt"),
    columns=1:25) annotation (Placement(transformation(extent={{92,72},{72,92}})));
  Modelica.Blocks.Sources.RealExpression Gdir(y=meaDat.y[2] - meaDat.y[3]) "[W/m2]"
    annotation (Placement(transformation(extent={{-31.5,
            44},{-12.5,60}})));
  Modelica.Blocks.Sources.RealExpression Gdif(y=meaDat.y[3]) "[W/m2]"
...
equation
  ...
  T_cell=TAmb.y+T_cell_init*eff_heat_loss*eff_wind_loss;
  G =Gtil.y;
  T_diff=T_cell-Tref;

  connect(Gdir.y, Gtil.u1)
    annotation (Line(points={{-11.55,52},{-6,52},{-6,22}}, color={0,0,127}));
  connect(Gdif.y, Gtil.u2)
    annotation (Line(points={{-11.55,66},{6,66},{6,22}}, color={0,0,127}));
  connect(solarPower.y, P) annotation (Line(points={{-10,-42},{-96,-42},{-96,0},
          {-110,0}}, color={0,0,127}));
    annotation (Dialog(group="Characteristics of the photovoltaic installation"),
