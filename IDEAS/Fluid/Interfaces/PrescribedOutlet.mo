...
"The parameter tau, or the volume of the model from which tau may be derived, is unreasonably small.
 ...
 if use_X_wSet then
  assert(Medium.nX > 1, "If use_X_wSet = true, require a medium with water vapor, such as IDEAS.Media.Air");
 end if;
...
equation
  ...
  m_flow_pos = IDEAS.Utilities.Math.Functions.smoothMax(
   ...
   if not restrictHeat and not restrictCool and
     ...
     m_flow_non_zero = IDEAS.Utilities.Math.Functions.smoothMax(
       x1 = port_a.m_flow,
       x2 = m_flow_small,
  ...
  if use_X_wSet then
    if not restrictHumi and not restrictDehu then
      ...
      if restrictHumi and restrictDehu then
        ...
        dXiAct =IDEAS.Utilities.Math.Functions.smoothLimit(
      ...
      elseif restrictHumi then
        ...
        dXiAct =IDEAS.Utilities.Math.Functions.smoothMin(
              x1=Xi - sum(Xi_instream),
              x2=mWatMax_flow/m_flow_non_zero,
              deltaX=deltaXi);
      else
        // Capacity limit for cooling only
        dXiAct =IDEAS.Utilities.Math.Functions.smoothMax(
              x1=Xi - sum(Xi_instream),
  ...
  if use_TSet then
    if not restrictHeat and not restrictCool then
      ...
      if restrictHeat and restrictCool then
        // Capacity limits for heating and cooling
        dhAct =IDEAS.Utilities.Math.Functions.smoothLimit(
      ...
      elseif restrictHeat then
        // Capacity limit for heating only
        dhAct =IDEAS.Utilities.Math.Functions.smoothMin(
              x1=hSet - inStream(port_a.h_outflow),
              x2=QMax_flow/m_flow_non_zero,
              deltaX=deltaH);
      else
        // Capacity limit for cooling only
        dhAct =IDEAS.Utilities.Math.Functions.smoothMax(
              x1=hSet - inStream(port_a.h_outflow),
...
See <a href=\"modelica://IDEAS.Fluid.HeatExchangers.PrescribedOutlet\">
IDEAS.Fluid.HeatExchangers.PrescribedOutlet</a>
