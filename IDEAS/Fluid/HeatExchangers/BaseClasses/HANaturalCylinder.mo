...
algorithm
  num := (0.387*IDEAS.Utilities.Math.Functions.smoothMax(Ra,1,0.1)^(1/6));
...
initial equation
  ...
  // Fluid properties
  mu_nominal = IDEAS.Fluid.HeatExchangers.BaseClasses.dynamicViscosityWater(
  ...
  rho_nominal = Medium.density(
        Medium.setState_pTX(
          ...
          T = 0.5*(TSur_nominal+TFlu_nominal),
          X = Medium.X_default));
  Pr_nominal = IDEAS.Fluid.HeatExchangers.BaseClasses.prandtlNumberWater(
          T = 0.5*(TSur_nominal+TFlu_nominal));

  B_nominal = IDEAS.Fluid.HeatExchangers.BaseClasses.isobaricExpansionCoefficientWater(
          T = 0.5*(TSur_nominal+TFlu_nominal));
...
equation
  ...
  state = Medium.setState_pTX(
             ...
             T = 0.5*(TSur+TFlu),
             X = Medium.X_default);
  mu = IDEAS.Fluid.HeatExchangers.BaseClasses.dynamicViscosityWater(
        T = 0.5 * (TSur+TFlu));
  rho = Medium.density(state);
  Pr = IDEAS.Fluid.HeatExchangers.BaseClasses.prandtlNumberWater(
          T = 0.5*(TSur+TFlu));

  B = IDEAS.Fluid.HeatExchangers.BaseClasses.isobaricExpansionCoefficientWater(
          T = 0.5*(TSur+TFlu));
  nu = mu/rho;
