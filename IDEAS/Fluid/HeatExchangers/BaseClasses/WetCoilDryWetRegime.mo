...
model WetCoilDryWetRegime
  ...
  Modelica.Blocks.Interfaces.RealOutput mCon_flow(
    ...
    annotation (
      Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=0,
        origin={150,-100})));
  IDEAS.Fluid.HeatExchangers.BaseClasses.WetCoilDryRegime fullyDry(
    final UAWat=UAWat,
  ...
  IDEAS.Fluid.HeatExchangers.BaseClasses.WetCoilWetRegime fullyWet(
    final UAWat=UAWat,
    final mWat_flow=mWat_flow,
    final cpWat=cpWat,
    final TWatIn=TWatIn,
...
protected
  ...
  IDEAS.Utilities.Psychrometrics.pW_X pWIn(
    final X_w=X_wAirIn,
    final p_in=pAir);
  IDEAS.Utilities.Psychrometrics.TDewPoi_pW TDewIn(
    final p_w=pWIn.p_w);
...
equation
  ...
  mu_FW= IDEAS.Utilities.Math.Functions.spliceFunction(
    pos=0,neg=1,x=fullyWet.TSurAirIn-TAirInDewPoi,
    deltax=IDEAS.Utilities.Math.Functions.smoothMax(
      abs(fullyDry.TSurAirOut-fullyWet.TSurAirIn), 1e-2,1e-3));

  mu_FD= IDEAS.Utilities.Math.Functions.spliceFunction(
  ...
  mCon_flow=QLat_flow/IDEAS.Utilities.Psychrometrics.Constants.h_fg*Qfac;
