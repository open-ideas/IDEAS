...
model WetCoilUARated
  ...
  replaceable package MediumA=IDEAS.Media.Air
    constrainedby Modelica.Media.Interfaces.PartialMedium
    "Air-side medium";
  replaceable package MediumW=IDEAS.Media.Water
    constrainedby Modelica.Media.Interfaces.PartialMedium
    "Water-side medium";
...
protected
  constant Modelica.Units.SI.SpecificEnthalpy hfg=IDEAS.Utilities.Psychrometrics.Constants.h_fg
  ...
  parameter Modelica.Units.SI.AbsolutePressure pSatTWatIn=
      IDEAS.Utilities.Psychrometrics.Functions.saturationPressure(TWatIn)
    "Saturation pressure of water at the water inlet temperature";
  parameter Modelica.Units.SI.MassFraction X_wSatTWatIn=
      IDEAS.Utilities.Psychrometrics.Functions.X_pSatpphi(
      pSat=pSatTWatIn,
      p=MediumA.p_default,
      phi=1)
  ...
  parameter Modelica.Units.SI.SpecificEnthalpy hSatTWatIn=
      IDEAS.Media.Air.specificEnthalpy_pTX(
      p=MediumA.p_default,
      T=TWatIn,
  ...
  parameter Modelica.Units.SI.AbsolutePressure pSatTWatOut=
      IDEAS.Utilities.Psychrometrics.Functions.saturationPressure(TWatOut)
  ...
  if use_Q_flow_nominal then
    if isFulDry then
      ...
      LMED = (if abs(TWatIn - TAirOut - (TWatOut - TAirIn)) < Modelica.Constants.small then
        TWatIn - TAirOut else IDEAS.Fluid.HeatExchangers.BaseClasses.lmtd(
          TWatIn,
          TWatOut,
          TAirIn,
          TAirOut)) / TUnit * hUnit;
      ...
      LMED = (if abs(hSatTWatIn - hAirOut - (hSatTWatOut - hAirIn)) < Modelica.Constants.small then
        ...
        IDEAS.Fluid.HeatExchangers.BaseClasses.lmtd(
