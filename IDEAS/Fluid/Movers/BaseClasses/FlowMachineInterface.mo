...
protected
  ...
  final parameter Real etaDer[size(per.efficiency.V_flow,1)]=
    if not per.etaHydMet==IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate
    ...
    elseif (size(per.efficiency.V_flow, 1) == 1)
      ...
      IDEAS.Utilities.Math.Functions.splineDerivatives(
        ...
        ensureMonotonicity=IDEAS.Utilities.Math.Functions.isMonotonic(
  ...
  final parameter Real motDer[size(per.motorEfficiency.V_flow, 1)]=
    if not per.etaMotMet==IDEAS.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate
  ...
  final parameter Real motDer_yMot_generic[9]=
    if per.etaMotMet==
      ...
      then IDEAS.Utilities.Math.Functions.splineDerivatives(
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.etaHydMet==
      IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
     IDEAS.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
  ...
  final parameter IDEAS.Fluid.Movers.BaseClasses.Euler.powerWithDerivative powEu_internal=
    if (curve == 1) then
      IDEAS.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur1)
    elseif (curve == 2) then
      IDEAS.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur2)
    else
      IDEAS.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur3)
    "Intermediate parameter";
  final parameter IDEAS.Fluid.Movers.BaseClasses.Characteristics.powerParameters powEu(
...
equation
  ...
  if (curve == 1) then
    if homotopyInitialization then
       V_flow*kRes + dp_internal = homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     V_flow=V_flow,
                                                     r_N=r_N,
                                                     dpMax=dpMax,
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1),
                                           simplified=r_N * (IDEAS.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1)
                               +(V_flow-V_flow_nominal) * (IDEAS.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1)
                                       -IDEAS.Fluid.Movers.BaseClasses.Characteristics.pressure(V_flow=(1-delta)*V_flow_nominal,
                                                     r_N=1,
                                                     dpMax=dpMax,
                                                     V_flow_max=V_flow_max,
                                                     d=preDer1,
       ...
       V_flow*kRes + dp_internal= IDEAS.Fluid.Movers.BaseClasses.Characteristics.pressure(V_flow=V_flow,
                                               r_N=r_N,
                                               dpMax=dpMax,
                                                     V_flow_max=V_flow_max,
                                               d=preDer1,
  ...
  WFlo = IDEAS.Utilities.Math.Functions.smoothMax(
  ...
  if per.powerOrEfficiencyIsHydraulic then
    ...
    etaHyd = IDEAS.Utilities.Math.Functions.smoothMin(
  ...
  if per.powerOrEfficiencyIsHydraulic then
    P_internal=WHyd;
    eta_internal=etaHyd;
    PEle = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(
                    x1=eta, x2=1E-2, deltaX=1E-3);
  else
    P_internal=PEle;
    eta_internal=eta;
    WHyd = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(
                    x1=etaHyd, x2=1E-2, deltaX=1E-3);
  end if;
  if per.etaHydMet==
       IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
    if homotopyInitialization then
      P_internal = homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow, r_N=r_N, d=powDer, delta=delta),
                      simplified=V_flow/V_flow_nominal*
                            IDEAS.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow_nominal, r_N=1, d=powDer, delta=delta));
    else
      P_internal = (rho/rho_default)*IDEAS.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow, r_N=r_N, d=powDer, delta=delta);
    end if;
    eta_internal = WFlo/IDEAS.Utilities.Math.Functions.smoothMax(
                     x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet==
       IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber then
    if homotopyInitialization then
      P_internal = homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow, r_N=r_N, d=powEuDer, delta=delta),
                      simplified=V_flow/V_flow_nominal*
                            IDEAS.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow_nominal, r_N=1, d=powEuDer, delta=delta));
    else
      P_internal = (rho/rho_default)*IDEAS.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow, r_N=r_N, d=powEuDer, delta=delta);
    end if;
    eta_internal = WFlo / IDEAS.Utilities.Math.Functions.smoothMax(
                            x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet == IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
      eta_internal = homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency,     V_flow=V_flow, d=etaDer, r_N=r_N, delta=delta),
                        simplified=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency, V_flow=V_flow_max,   d=etaDer, r_N=r_N, delta=delta));
    else
      eta_internal = IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency, V_flow=V_flow, d=etaDer, r_N=r_N, delta=delta);
    end if;
    P_internal=WFlo/IDEAS.Utilities.Math.Functions.smoothMax(
  ...
  if per.etaMotMet == IDEAS.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
      etaMot = homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency,     V_flow=V_flow, d=motDer, r_N=r_N, delta=delta),
                        simplified=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency, V_flow=V_flow_max,   d=motDer, r_N=r_N, delta=delta));
    else
      etaMot = IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency,     V_flow=V_flow, d=motDer, r_N=r_N, delta=delta);
    end if;
  elseif per.etaMotMet==
       IDEAS.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio then
    if homotopyInitialization then
      etaMot =homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=yMot,
        d=motDer_yMot), simplified=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=1,
        d=motDer_yMot));
    else
      etaMot =IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=yMot,
        d=motDer_yMot);
    end if;
  elseif per.etaMotMet==
       IDEAS.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve then
      if homotopyInitialization then
        etaMot =homotopy(actual=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
          per=per.motorEfficiency_yMot_generic,
          y=yMot,
          d=motDer_yMot_generic), simplified=IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
          per=per.motorEfficiency_yMot_generic,
          y=1,
...
IDEAS.Fluid.Movers.UsersGuide</a>
...
<a href=\"modelica://IDEAS.Fluid.Movers.BaseClasses.PowerInterface\">
IDEAS.Fluid.Movers.BaseClasses.PowerInterface</a>.
</li>
<li>
...
<code>etaMot = IDEAS.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency, V_flow=V_flow, d=motDer, r_N=r_N, delta=1E-4)</code>
