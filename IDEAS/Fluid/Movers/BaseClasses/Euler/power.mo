...
function power
  ...
  input IDEAS.Fluid.Movers.BaseClasses.Euler.peak peak "Peak operation point";
  ...
  output IDEAS.Fluid.Movers.BaseClasses.Euler.powerWithDerivative power(
...
algorithm
  ...
  for i in 2:10 loop
    power.P[i]:=V_flow[i] * dp[i]
                / IDEAS.Fluid.Movers.BaseClasses.Euler.efficiency(
                    peak=peak,
                    dp=dp[i],
                    V_flow=V_flow[i],
  ...
  power.d[2:10]:=IDEAS.Utilities.Math.Functions.splineDerivatives(
                   x=V_flow[2:10],
                   y=power.P[2:10],
                   ensureMonotonicity=false);
