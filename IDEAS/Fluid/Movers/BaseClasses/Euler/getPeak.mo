within IDEAS.Fluid.Movers.BaseClasses.Euler;
function getPeak
  "Find peak condition from power characteristics"
  extends Modelica.Icons.Function;
  input IDEAS.Fluid.Movers.BaseClasses.Characteristics.flowParameters
    pressure "Pressure vs. flow rate";
  input BaseClasses.Characteristics.powerParameters
    power "Power vs. flow rate";
  output IDEAS.Fluid.Movers.BaseClasses.Euler.peak
...
protected
  ...
  parameter Modelica.Units.SI.PressureDifference dpHalFlo=
    if max(pressure.V_flow) < 1E-6
      then 0
    else IDEAS.Utilities.Math.Functions.smoothInterpolation(
               x=V_flow_hal,
  ...
  elseif max(power.P) < 1E-6 then
    ...
    if nPre == nPow then
      ...
      if nPre > nPow then
        ...
        dp:= pressure.dp;
        for i in 1:n loop
          P[i]:= IDEAS.Utilities.Math.Functions.smoothInterpolation(
                   x=V_flow[i],
                   xSup=power.V_flow,
                   ySup=power.P,
        ...
        P:= power.P;
        for i in 1:n loop
          dp[i]:= IDEAS.Utilities.Math.Functions.smoothInterpolation(
                    x=V_flow[i],
                    xSup=pressure.V_flow,
                    ySup=pressure.dp,
                    ensureMonotonicity=true);
    ...
    eta:=V_flow.*dp./P;
    etaLes:=n<4;
    etaMon:=IDEAS.Utilities.Math.Functions.isMonotonic(
      x=eta,
      strict=false);
    ...
    //   2. the available point with the highest efficiency.
      peak.eta:=IDEAS.Utilities.Math.Functions.smoothInterpolation(
               x=V_flow_hal,
               xSup=V_flow,
               ySup=eta,
               ensureMonotonicity=
                 IDEAS.Utilities.Math.Functions.isMonotonic(eta, strict=false));
      ...
      peak.eta:=IDEAS.Utilities.Math.Functions.smoothInterpolation(
        ...
        ySup=eta,
        ensureMonotonicity=false);
      peak.dp:=IDEAS.Utilities.Math.Functions.smoothInterpolation(
        x=peak.V_flow,
        xSup=V_flow,
        ySup=dp,
        ensureMonotonicity=false);
...
<a href=\"modelica://IDEAS.Fluid.Movers.BaseClasses.Euler.peak\">
IDEAS.Fluid.Movers.BaseClasses.Euler.peak</a>.
