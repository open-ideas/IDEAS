...
model FlowControlled_dp
  ...
  extends IDEAS.Fluid.Movers.BaseClasses.PartialFlowMachine(
    ...
    eff(per(
        ...
        final etaHydMet=if (per.etaHydMet == IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate
          or per.etaHydMet == IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber)
          and not per.havePressureCurve then
            IDEAS.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.NotProvided
          else
            per.etaHydMet,
        final etaMotMet=if (per.etaMotMet == IDEAS.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
          or per.etaMotMet == IDEAS.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve)
  ...
  annotation (
    Icon(graphics={
        Text(
          ...
          textString=DynamicSelect("dp", if inputType == IDEAS.Fluid.Types.InputType.Continuous then String(dp_in, format=".0f") else String(stage)))}),
...
<a href=\"modelica://IDEAS.Obsolete.Fluid.Movers.FlowControlled_Nrpm\">
IDEAS.Obsolete.Fluid.Movers.FlowControlled_Nrpm</a>
and
