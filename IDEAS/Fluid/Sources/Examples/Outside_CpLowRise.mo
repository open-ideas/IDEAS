...
model Outside_CpLowRise
  ...
  IDEAS.Fluid.Sources.Outside_CpLowRise west(
  ...
  IDEAS.Fluid.Sources.Outside_CpLowRise north(
    redeclare package Medium = Medium,
    s=1/5,
    azi=IDEAS.Types.Azimuth.N,
    Cp0=0.6) "Model with outside conditions"
    annotation (Placement(transformation(extent={{-20,40},{0,60}})));
  IDEAS.Fluid.Sources.Outside_CpLowRise south(
    redeclare package Medium = Medium,
    s=1/5,
    azi=IDEAS.Types.Azimuth.S,
    Cp0=0.6) "Model with outside conditions"
    annotation (Placement(transformation(extent={{-20,-40},{0,-20}})));
  IDEAS.Fluid.Sources.Outside_CpLowRise east(
    redeclare package Medium = Medium,
    s=5,
...
equation
  ...
  connect(weaDat.weaBus, east.weaBus) annotation (Line(
      ...
      thickness=0.5,
      smooth=Smooth.None));
  annotation (__Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Fluid/Sources/Examples/Outside_CpLowRise.mos"
        "Simulate and plot"),
