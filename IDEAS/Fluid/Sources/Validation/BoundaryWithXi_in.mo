...
model BoundaryWithXi_in
  ...
  IDEAS.Fluid.Sources.Validation.BaseClasses.BoundarySystemWithXi_in bouMoiAir(
    redeclare package Medium = IDEAS.Media.Air)
    "Boundary with moist air"
    annotation (Placement(transformation(extent={{20,70},{40,90}})));

  IDEAS.Fluid.Sources.Validation.BaseClasses.BoundarySystemWithXi_in bouMoiAirCO2(
    redeclare package Medium = IDEAS.Media.Air(extraPropertiesNames={"CO2"}))
    "Boundary with moist air"
    annotation (Placement(transformation(extent={{20,30},{40,50}})));

  IDEAS.Fluid.Sources.Validation.BaseClasses.BoundarySystemWithXi_in bouProFluGas(
   redeclare package Medium =
        Modelica.Media.IdealGases.MixtureGases.FlueGasSixComponents)
    "Boundary with flue gas"
    annotation (Placement(transformation(extent={{20,-10},{40,10}})));

  IDEAS.Fluid.Sources.Validation.BaseClasses.BoundarySystemWithXi_in bouNatGas(
    redeclare package Medium =
...
__Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Fluid/Sources/Validation/BoundaryWithXi_in.mos"
        "Simulate and plot"),
experiment(
      StopTime=1,
      Tolerance=1e-06));
