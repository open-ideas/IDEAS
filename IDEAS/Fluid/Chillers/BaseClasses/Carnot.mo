within IDEAS.Fluid.Chillers.BaseClasses;
partial model Carnot
  ...
  Real etaPL(final unit = "1")=
    if evaluate_etaPL
      then IDEAS.Utilities.Math.Functions.polynomial(a=a, x=yPL)
      else 1
  ...
  Real COPCar(min=0) = TUseAct/IDEAS.Utilities.Math.Functions.smoothMax(
    x1=1,
    x2=TConAct - TEvaAct,
...
initial equation
  ...
  assert(abs(IDEAS.Utilities.Math.Functions.polynomial(
