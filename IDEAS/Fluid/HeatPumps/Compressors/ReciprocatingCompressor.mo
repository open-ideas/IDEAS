within IDEAS.Fluid.HeatPumps.Compressors;
model ReciprocatingCompressor
  "Model for a reciprocating compressor, based on Jin (2002)"
  extends IDEAS.Fluid.HeatPumps.Compressors.BaseClasses.PartialCompressor;
...
equation
  ...
  pisDis_norm = IDEAS.Utilities.Math.Functions.smoothLimit(y, 0.0, 1.0, 0.001);

  if isOn then
    // Suction pressure
    pSuc = IDEAS.Utilities.Math.Functions.smoothMin(pEva - pDro, pCon - pDro, 0.01*ref.pCri);
    // Discharge pressure
    pDis = pCon + pDro;
