...
protected
  ...
  Integer i "Integer to select data interval based on asked power";
  Integer i_mod "Integer to select the modulation interval";
public
...
algorithm
  ...
  modulationInit := IDEAS.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
    x=QAsked,
    x1=QVector[i],
    x2=QVector[i + 1],
    y1=modVector[i],
  ...
  for j in 1:6-1 loop
    if modulation > modVector[j] then
      i_mod:= j;
    end if;
  end for;
  eta :=Modelica.Fluid.Utilities.cubicHermite(
    x=modulation,
    x1=modVector[i_mod],
    x2=modVector[i_mod + 1],
    y1=etaVector[i_mod],
    y2=etaVector[i_mod + 1],
    y1d=0,
    y2d=0);
  heatPort.Q_flow :=-
