...
model CausalInhomeFeeder
  ...
  Experimental.Electric.BaseClasses.AC.WattsLaw wattsLaw(numPha=1)
    annotation (Placement(transformation(extent={{20,-10},{40,10}})));

  Experimental.Electric.Distribution.AC.BaseClasses.BranchLenTyp branch(len=len)
    "Cable to district feeder"
    annotation (Placement(transformation(extent={{60,-10},{80,10}})));
  IDEAS.Experimental.Electric.BaseClasses.AC.PotentialSensor voltageSensor
...
protected
  Modelica.Electrical.QuasiStatic.SinglePhase.Sources.VoltageSource
    ...
    V=230,
    phi=0,
    gamma(fixed=true)) "Steady building-side 230 V voltage source" annotation (
      Placement(transformation(
        extent={{-8,-8},{8,8}},
        rotation=270,
        origin={-80,-42})));
  Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Ground ground(pin(final
        reference)) "Grounding for the building-side voltage source"
