...
model OneEffectiveAirLeakageArea "Model with an effective air leakage area"
  ...
  Modelica.Blocks.Math.Gain Gain1(k=100) annotation (Placement(transformation(
          extent={{-40,20},{-20,40}})));
  IDEAS.Airflow.Multizone.EffectiveAirLeakageArea cra(redeclare package Medium
      =        Medium, L=20E-4)
...
equation
  ...
  connect(Gain1.u, Sine1.y) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  annotation (__Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Airflow/Multizone/Examples/OneEffectiveAirLeakageArea.mos"
        "Simulate and plot"),
        experiment(
      StopTime=7200,
      Tolerance=1e-05),
