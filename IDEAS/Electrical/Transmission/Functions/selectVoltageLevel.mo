...
function selectVoltageLevel
  ...
  output IDEAS.Electrical.Types.VoltageLevel level "Type of voltage level";
algorithm
  if V <= 0 then
    assert(V > 0,
     "In function IDEAS.Electrical.Transmission.Functions.selectVoltageLevel,
      does not support a voltage of " + String(V) + " [V].
      The selected voltage level will be assumed to be low.",
      level = AssertionLevel.warning);
    level := IDEAS.Electrical.Types.VoltageLevel.Low;
  elseif V <= 1000 then
    level := IDEAS.Electrical.Types.VoltageLevel.Low;
  elseif V > 1000 and V <= 50000 then
    level := IDEAS.Electrical.Types.VoltageLevel.Medium;
  else
    level := IDEAS.Electrical.Types.VoltageLevel.High;
  end if;
