within IDEAS.Electrical.AC.OnePhase.Conversion;
model ACACTransformerFull "AC AC transformer with detailed equivalent circuit"
  extends IDEAS.Electrical.Icons.RefAngleConversion;
  extends IDEAS.Electrical.Interfaces.PartialConversion(
  ...
  parameter IDEAS.Electrical.Types.PerUnit Rm(min=0)
    "Magnetization resistance (pu)"
    annotation(Evaluate=true, Dialog(group="Magnetization", enable = magEffects));
  parameter IDEAS.Electrical.Types.PerUnit Lm(min=0)
...
equation
  ...
  // Efficiency
  eta = IDEAS.Utilities.Math.Functions.smoothMin(
        x1=
  ...
  if magEffects then
    Im = IDEAS.Electrical.PhaseSystems.OnePhase.divide(V1, Zrm) +
         IDEAS.Electrical.PhaseSystems.OnePhase.divide(V1, Zlm);
  else
    Im = zeros(2);
  end if;

  // Losses due to the impedance - primary side
  terminal_n.v = V1 + IDEAS.Electrical.PhaseSystems.OnePhase.product(
    terminal_n.i, Z1);

  // Losses due to the impedance - secondary side
  terminal_p.v = V2 + IDEAS.Electrical.PhaseSystems.OnePhase.product(
    terminal_p.i, Z2);
