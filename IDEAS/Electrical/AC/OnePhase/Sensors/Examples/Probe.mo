within IDEAS.Electrical.AC.OnePhase.Sensors.Examples;
model Probe "This example illustrates how to use the probe model"
  extends Modelica.Icons.Example;
  IDEAS.Electrical.AC.OnePhase.Loads.Capacitive loaRC(
    mode=IDEAS.Electrical.Types.Load.FixedZ_steady_state,
    P_nominal=-10000,
    V_nominal=120) "Constant load"
    annotation (Placement(transformation(extent={{10,0},{30,20}})));
  IDEAS.Electrical.AC.OnePhase.Sources.FixedVoltage sou(f=60, V=120)
    "Voltage source"
    annotation (Placement(transformation(extent={{-60,-30},{-40,-10}})));
  IDEAS.Electrical.AC.OnePhase.Lines.TwoPortResistance res1(R=0.1)
    "First line resistance"
    annotation (Placement(transformation(extent={{-26,0},{-6,20}})));
  IDEAS.Electrical.AC.OnePhase.Sensors.Probe probe_source(V_nominal=120)
    "Probe that measures at the voltage source"
    annotation (Placement(transformation(extent={{-50,30},{-30,50}})));
  IDEAS.Electrical.AC.OnePhase.Sensors.Probe probe_loadRC(V_nominal=120)
    "Probe that measures at the RC load"
    annotation (Placement(transformation(extent={{-10,30},{10,50}})));
  IDEAS.Electrical.AC.OnePhase.Lines.TwoPortResistance res2(R=0.1)
    "Second line resistance"
    annotation (Placement(transformation(extent={{-26,-70},{-6,-50}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Inductive loaRL(
    mode=IDEAS.Electrical.Types.Load.FixedZ_steady_state, P_nominal=-10000,
    V_nominal=120) "Constant load"
    annotation (Placement(transformation(extent={{10,-70},{30,-50}})));
  IDEAS.Electrical.AC.OnePhase.Sensors.Probe probe_loadRL(V_nominal=120)
...
__Dymola_Commands(file=
          "modelica://IDEAS/Resources/Scripts/Dymola/Electrical/AC/OnePhase/Sensors/Examples/Probe.mos"
        "Simulate and plot"));
end Probe;
