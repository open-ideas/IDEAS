  ...
  Modelica.Blocks.Interfaces.RealOutput theta(unit="rad", displayUnit="deg")
    "Voltage phasor angle"
        ...
        transformation(extent={{60,-40},{80,-20}}), iconTransformation(extent={{60,
            -40},{80,-20}})));
equation
  theta = IDEAS.Electrical.PhaseSystems.OnePhase.phase(term.v);
  if perUnit then
    V = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(term.v)/V_nominal;
  else
    V = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(term.v);
  end if;
  term.i = zeros(IDEAS.Electrical.PhaseSystems.OnePhase.n);
  annotation (
