...
model TestImpedance "Example that illustrates the use of the impedances"
  ...
  IDEAS.Electrical.AC.OnePhase.Sources.FixedVoltage V(f=60, V=120)
           annotation (Placement(transformation(extent={{-80,20},{-60,40}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Z1(R=0,
    inductive=true,
    L=1/(2*Modelica.Constants.pi*60)) "Inductive impedance"
    annotation (Placement(transformation(extent={{-20,20},{0,40}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Z2(R=0,
    inductive=false,
    C=1/(2*Modelica.Constants.pi*60)) "Capacitive impedance"
    annotation (Placement(transformation(extent={{-20,0},{0,20}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Z3(R=1)
    "Resistive impedance"
    annotation (Placement(transformation(extent={{-20,-20},{0,0}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Z4(
    R=1,
    L=1/(2*Modelica.Constants.pi*60)) "Inductive-resistive impedance"
    annotation (Placement(transformation(extent={{-20,-40},{0,-20}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Z5(
    R=1,
    inductive=false,
...
equation
  ...
  annotation (experiment(StopTime=1.0, Tolerance=1e-06),
    __Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Electrical/AC/OnePhase/Loads/Examples/TestImpedance.mos"
        "Simulate and plot"),
