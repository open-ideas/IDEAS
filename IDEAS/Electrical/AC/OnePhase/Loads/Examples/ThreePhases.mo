...
model ThreePhases
  ...
  IDEAS.Electrical.AC.OnePhase.Sources.FixedVoltage Va(
    definiteReference=true,
    f=60,
    V=120) "Source phase A"
           annotation (Placement(transformation(extent={{-40,40},{-20,60}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Za(
    inductive=true,
    L=1/(2*Modelica.Constants.pi*60),
    R=12) "Impedance phase A"
    annotation (Placement(transformation(extent={{20,40},{40,60}})));
  IDEAS.Electrical.AC.OnePhase.Sources.FixedVoltage Vb(
    definiteReference=true, phiSou=-2.0943951023932,
    f=60,
    V=120) "Source phase B"
           annotation (Placement(transformation(extent={{-40,0},{-20,20}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Zb(
    inductive=true,
    L=1/(2*Modelica.Constants.pi*60),
    R=12) "Impedance phase B"
    annotation (Placement(transformation(extent={{20,0},{40,20}})));
  IDEAS.Electrical.AC.OnePhase.Sources.FixedVoltage Vc(
    definiteReference=true, phiSou=2.0943951023932,
    f=60,
    V=120) "Source phase C"
           annotation (Placement(transformation(extent={{-40,-40},{-20,
            -20}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Impedance Zc(
    inductive=true,
    L=1/(2*Modelica.Constants.pi*60),
...
equation
  ...
  annotation (experiment(StopTime=1.0, Tolerance=1e-06),
    __Dymola_Commands(file=
          "modelica://IDEAS/Resources/Scripts/Dymola/Electrical/AC/OnePhase/Loads/Examples/ThreePhases.mos"
        "Simulate and plot"),
