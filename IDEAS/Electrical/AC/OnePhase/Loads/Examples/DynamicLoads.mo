...
model DynamicLoads "Example that illustrates the use of dynamic loads"
  ...
  IDEAS.Electrical.AC.OnePhase.Sources.FixedVoltage source(
    f=60,
    V=120) "Voltage source" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        origin={-70,10})));
  IDEAS.Electrical.AC.OnePhase.Loads.Capacitive dynRC(
    pf=0.8,
    mode=IDEAS.Electrical.Types.Load.FixedZ_dynamic,
    P_nominal=-1200,
    V_nominal=120) "Dynamic RC load"
    annotation (Placement(transformation(extent={{0,-20},{20,0}})));
  IDEAS.Electrical.AC.OnePhase.Lines.TwoPortResistance line(R=0.1)
    "Line resistance"
    annotation (Placement(transformation(extent={{-40,0},{-20,20}})));
  IDEAS.Electrical.AC.OnePhase.Loads.Inductive dynRL(
    pf=0.8,
    mode=IDEAS.Electrical.Types.Load.FixedZ_dynamic,
    P_nominal=-1200,
...
First implementation.
    ...
    __Dymola_Commands(file=
          "modelica://IDEAS/Resources/Scripts/Dymola/Electrical/AC/OnePhase/Loads/Examples/DynamicLoads.mos"
        "Simulate and plot"));
end DynamicLoads;
