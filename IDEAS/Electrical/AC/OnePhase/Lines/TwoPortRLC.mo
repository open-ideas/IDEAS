within IDEAS.Electrical.AC.OnePhase.Lines;
model TwoPortRLC "Model of an RLC element with two electrical ports"
  extends IDEAS.Electrical.Transmission.BaseClasses.PartialTwoPortRLC(
    V_nominal(start = 110),
  ...
  parameter Modelica.Units.SI.Voltage Vc_start[2]={V_nominal,0}
    ...
    annotation (Dialog(enable=(mode == IDEAS.Electrical.Types.Load.FixedZ_dynamic)));
  parameter IDEAS.Electrical.Types.Load mode(
    min=IDEAS.Electrical.Types.Load.FixedZ_steady_state,
    max=IDEAS.Electrical.Types.Load.FixedZ_dynamic)=
    IDEAS.Electrical.Types.Load.FixedZ_steady_state
...
initial equation
  if C > 0 and mode == IDEAS.Electrical.Types.Load.FixedZ_dynamic then
    Vc = Vc_start;
  end if;
equation
  ...
  L/2*omega*IDEAS.Electrical.PhaseSystems.OnePhase.j(terminal_p.i) +
    terminal_p.i*diagonal(ones(PhaseSystem_p.n)*R_actual/2) = terminal_p.v - Vc;
  L/2*omega*IDEAS.Electrical.PhaseSystems.OnePhase.j(terminal_n.i) +
  ...
  if C > 0 then
    if mode == IDEAS.Electrical.Types.Load.FixedZ_dynamic then
      ...
      C*der(Vc) + omega*C*IDEAS.Electrical.PhaseSystems.OnePhase.j(Vc) = Ic;
    else
      // steady state relationship
      omega*C*IDEAS.Electrical.PhaseSystems.OnePhase.j(Vc) = Ic;
    end if;
  else
...
<p align=\"center\">
<img alt=\"image\" src=\"modelica://IDEAS/Resources/Images/Electrical/AC/OnePhase/Lines/twoPortRLC.png\"/>
</p>
<p>
...
<a href=\"modelica://IDEAS.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortRLC\">
IDEAS.Electrical.AC.ThreePhasesBalanced.Lines.TwoPortRLC</a>.
</li>
<li>
