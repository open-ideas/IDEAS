...
model Network "Single phase AC network"
  extends IDEAS.Electrical.Transmission.BaseClasses.PartialNetwork(
    V_nominal(start = 110),
  ...
  parameter IDEAS.Electrical.Types.Load modelMode=Types.Load.FixedZ_steady_state
    "Select between steady state and dynamic model" annotation (Dialog(
      tab="Model",
      group="Assumptions",
      enable=use_C), choices(choice=IDEAS.Electrical.Types.Load.FixedZ_steady_state
...
equation
  ...
  for i in 1:grid.nNodes loop
    VAbs[i] = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(terminal[i].v);
  end for;
