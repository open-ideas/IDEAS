...
equation
  for i in 1:3 loop
      theta[i] = IDEAS.Electrical.PhaseSystems.OnePhase.phase(wyeToDelta.delta.phase[i].v);
      if perUnit then
        V[i] = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(wyeToDelta.delta.phase[i].v)/V_nominal;
      else
        V[i] = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(wyeToDelta.delta.phase[i].v);
      end if;
  end for;
