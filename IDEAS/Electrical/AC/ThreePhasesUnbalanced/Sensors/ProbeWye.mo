within IDEAS.Electrical.AC.ThreePhasesUnbalanced.Sensors;
model ProbeWye
  ...
  IDEAS.Electrical.AC.ThreePhasesUnbalanced.Interfaces.Terminal_n
...
equation
  for i in 1:3 loop
      theta[i] = IDEAS.Electrical.PhaseSystems.OnePhase.phase(wyeToWyeGround.wyeg.phase[i].v);
      if perUnit then
        V[i] = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(wyeToWyeGround.wyeg.phase[i].v)/(V_nominal/sqrt(3));
      else
        V[i] = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(wyeToWyeGround.wyeg.phase[i].v);
      end if;
  end for;
