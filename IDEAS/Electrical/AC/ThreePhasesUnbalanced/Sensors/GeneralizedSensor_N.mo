...
model GeneralizedSensor_N
  ...
  Modelica.Blocks.Interfaces.RealOutput I[4](each final quantity="ElectricCurrent",
   ...
   annotation (Placement(
        ...
        rotation=270,
        origin={60,-90})));
  Modelica.Blocks.Interfaces.RealOutput S[4, IDEAS.Electrical.PhaseSystems.OnePhase.n](
...
equation
  ...
  for i in 1:4 loop
    V[i]   = IDEAS.Electrical.PhaseSystems.OnePhase.systemVoltage(terminal_n.phase[i].v);
    I[i]   = IDEAS.Electrical.PhaseSystems.OnePhase.systemCurrent(terminal_n.phase[i].i);
    S[i,:] = IDEAS.Electrical.PhaseSystems.OnePhase.phasePowers_vi(v=terminal_n.phase[i].v, i=terminal_n.phase[i].i);
  end for;
