...
model GeneralizedSensor "Example model for generalized sensor"
  ...
  IDEAS.Electrical.AC.ThreePhasesBalanced.Sensors.GeneralizedSensor
    sen "Sensor model"
    annotation (Placement(transformation(extent={{-20,2},{0,22}})));
  IDEAS.Electrical.AC.ThreePhasesBalanced.Loads.Capacitive loa(
                   V_nominal=480, P_nominal=-100) "Constant load"
    annotation (Placement(transformation(extent={{40,2},{60,22}})));
  IDEAS.Electrical.AC.ThreePhasesBalanced.Sources.FixedVoltage
...
__Dymola_Commands(file=
          "modelica://IDEAS/Resources/Scripts/Dymola/Electrical/AC/ThreePhasesBalanced/Sensors/Examples/GeneralizedSensor.mos"
        "Simulate and plot"));
