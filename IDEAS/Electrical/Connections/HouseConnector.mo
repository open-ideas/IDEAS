within IDEAS.Electrical.Connections;
model HouseConnector
  ...
  replaceable parameter IDEAS.Electrical.Data.Interfaces.Cable typHouBran=
...
Maybe also make sure unused cables have very high impedance so errors will show in results***/
  IDEAS.Electrical.Distribution.BaseClasses.BranchLenTyp[numBran] branch(
    each len=lenHouBran,
    each typ=typHouBran,
    each mulFac=mulFacZ);
