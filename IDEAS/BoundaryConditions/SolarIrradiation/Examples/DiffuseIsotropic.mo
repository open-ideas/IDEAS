  ...
  IDEAS.BoundaryConditions.WeatherData.Bus weaBus "Weather data bus"
    annotation (Placement(
        transformation(extent={{-11,-1},{9,21}}),iconTransformation(extent={{1,
            -1},{2,-2}})));
  IDEAS.BoundaryConditions.SolarIrradiation.DiffuseIsotropic
        HDifRoo(til=IDEAS.Types.Tilt.Ceiling,
                rho=rho) "Diffuse irradiation on roof"
    annotation (Placement(transformation(extent={{40,0},{60,20}})));
  IDEAS.BoundaryConditions.SolarIrradiation.DiffuseIsotropic
        HDifFlo(til=IDEAS.Types.Tilt.Floor,
...
__Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/BoundaryConditions/SolarIrradiation/Examples/DiffuseIsotropic.mos"
        "Simulate and plot"));
