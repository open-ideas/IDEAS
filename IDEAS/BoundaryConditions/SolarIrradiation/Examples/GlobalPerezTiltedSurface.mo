...
model GlobalPerezTiltedSurface
  ...
  IDEAS.BoundaryConditions.SolarIrradiation.GlobalPerezTiltedSurface HGloRoo(
    til=IDEAS.Types.Tilt.Ceiling,
    azi=0.78539816339745,
    rho=rho)
    "Diffuse irradiation on roof"
    annotation (Placement(transformation(extent={{40,0},{60,20}})));
  IDEAS.BoundaryConditions.SolarIrradiation.GlobalPerezTiltedSurface HGloFlo(
    til=IDEAS.Types.Tilt.Floor,
    azi=0.78539816339745,
    rho=rho)
    "Diffuse irradiation on floor"
    annotation (Placement(transformation(extent={{40,-80},{60,-60}})));
  IDEAS.BoundaryConditions.SolarIrradiation.GlobalPerezTiltedSurface HGloWal(
...
equation
  ...
  __Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/BoundaryConditions/SolarIrradiation/Examples/GlobalPerezTiltedSurface.mos"
        "Simulate and plot"),
