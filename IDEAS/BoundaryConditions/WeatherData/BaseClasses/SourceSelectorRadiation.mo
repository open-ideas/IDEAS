...
block SourceSelectorRadiation
  ...
  parameter IDEAS.BoundaryConditions.Types.RadiationDataSource datSou "Data source"
    annotation(Evaluate=true);

  Modelica.Blocks.Interfaces.RealInput HDirNorFil(
    final quantity="RadiantEnergyFluenceRate",
    final unit="W/m2")
   if datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.File
  ...
  Modelica.Blocks.Interfaces.RealInput HDirNorIn(
    final quantity="RadiantEnergyFluenceRate",
    final unit="W/m2")
   if datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor or
      datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor
  ...
  Modelica.Blocks.Interfaces.RealInput HDifHorIn(
    final quantity="RadiantEnergyFluenceRate",
    final unit="W/m2")
  if datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor or
     datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor
...
equation
  ...
  if datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor then
  ...
  if datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor then
  ...
  if datSou == IDEAS.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor then
      HDirNor = max(0, min(
        solCon,
        (HGloHor_in_internal -HDifHor_in_internal)*
          IDEAS.Utilities.Math.Functions.spliceFunction(
            x=cos(zen),
            pos=IDEAS.Utilities.Math.Functions.inverseXRegularized(cos(zen), epsCos),
            neg=0,
            deltax=epsCos)))
