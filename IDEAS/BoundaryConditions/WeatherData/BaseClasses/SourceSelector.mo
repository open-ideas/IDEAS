...
block SourceSelector
  ...
  parameter IDEAS.BoundaryConditions.Types.DataSource datSou "Data source"
    annotation(Evaluate=true);
  parameter Real p "Parameter value";
  Modelica.Blocks.Interfaces.RealInput uFil
   if datSou == IDEAS.BoundaryConditions.Types.DataSource.File
    "Input signal from file reader"
    annotation (Placement(transformation(extent={{-140,-80},{-100,-40}}),
        iconTransformation(extent={{-120,-90},{-100,-70}})));
  Modelica.Blocks.Interfaces.RealInput uCon
  if datSou == IDEAS.BoundaryConditions.Types.DataSource.Input
    "Input signal from input connector"
    annotation (Placement(transformation(extent={{-140,40},{-100,80}}),
        iconTransformation(extent={{-120,70},{-100,90}})));
equation
  if datSou == IDEAS.BoundaryConditions.Types.DataSource.Parameter then
    y = p;
  end if;
  connect(uCon, y);
  connect(uFil, y);
    ...
    Icon(graphics={
      Rectangle(
        ...
        Line(points={{-38,80},{6,2}},
          color={0,0,127},
          visible=datSou == IDEAS.BoundaryConditions.Types.DataSource.File,
          thickness=1),
        Line(points={{-40,-80},{8,0}},
          color={0,0,127},
          visible=datSou == IDEAS.BoundaryConditions.Types.DataSource.Input,
          thickness=1),
