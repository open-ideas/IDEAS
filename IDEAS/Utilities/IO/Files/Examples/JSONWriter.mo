...
model JSONWriter "Example use of the JSON writer"
  ...
  IDEAS.Utilities.IO.Files.JSONWriter jsonWriterInitial(
    nin=3,
    fileName="InitialOutputs.json",
    outputTime=IDEAS.Utilities.IO.Files.BaseClasses.OutputTime.Initial)
    "Outputs at initial time"
    annotation (Placement(transformation(extent={{0,20},{20,40}})));
  IDEAS.Utilities.IO.Files.JSONWriter jsonWriterCustom(
    nin=3,
    outputTime=IDEAS.Utilities.IO.Files.BaseClasses.OutputTime.Custom,
    customTime=0.5,
    fileName="CustomOutputs.json",
    varKeys={"Output1","Key2","Name3"}) "Outputs at custom time"
    annotation (Placement(transformation(extent={{0,-10},{20,10}})));
  IDEAS.Utilities.IO.Files.JSONWriter jsonWriterTerminal(
    nin=3,
  ...
  IDEAS.Utilities.IO.Files.JSONWriter jsonWriterOneVar(nin=1, fileName="TerminalOutput.json")
...
See <a href=\"https://github.com/ibpsa/modelica-ibpsa/issues/1114\">#1114</a>.
    ...
    experiment(
      StopTime=2,
      Tolerance=1e-06),
    __Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Utilities/IO/Files/Examples/JSONWriter.mos"
        "Simulate and plot"));
end JSONWriter;
