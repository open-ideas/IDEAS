...
algorithm
  ...
  y := IDEAS.Utilities.Math.Functions.smoothInterpolation(
    x=x,
    xSup=xSup,
    ySup=ySup,
    ensureMonotonicity=false);
  yMonotone := IDEAS.Utilities.Math.Functions.smoothInterpolation(
    x=x,
    xSup=xSup,
    ySup=ySup,
    ensureMonotonicity=true);
  // Case with n = 2
  y2 := IDEAS.Utilities.Math.Functions.smoothInterpolation(
    x=x,
    xSup=xSup[1:2],
    ySup=ySup[1:2],
    ensureMonotonicity=false);
  y2Monotone := IDEAS.Utilities.Math.Functions.smoothInterpolation(
    x=x,
    xSup=xSup[1:2],
    ySup=ySup[1:2],
    ensureMonotonicity=true);
  // Case with n = 1
  y1 := IDEAS.Utilities.Math.Functions.smoothInterpolation(
    x=x,
    xSup={xSup[1]},
    ySup={ySup[1]},
    ensureMonotonicity=false);
  y1Monotone := IDEAS.Utilities.Math.Functions.smoothInterpolation(
    x=x,
    xSup={xSup[1]},
    ySup={ySup[1]},
    ensureMonotonicity=true);
  annotation (
    __Dymola_Commands(file=
          "modelica://IDEAS/Resources/Scripts/Dymola/Utilities/Math/Functions/Examples/SmoothInterpolation.mos"
        "Simulate and plot"),
