...
equation
  ...
  phi = IDEAS.Utilities.Psychrometrics.Functions.phi_pTX(p=p, T=T, X_w=X_w);
  X_inv=IDEAS.Utilities.Psychrometrics.Functions.X_pTphi(p=p, T=T, phi=phi);
  assert(abs(X_inv-X_w) < 1e-5,
    "Inconsistency in the inverse implementation of phi_pTX: X_pTphi");
  annotation (
experiment(Tolerance=1e-6, StopTime=1.0),
__Dymola_Commands(file="modelica://IDEAS/Resources/Scripts/Dymola/Utilities/Psychrometrics/Functions/Examples/phi_pTX.mos"
        "Simulate and plot"),
