...
package Air
  ...
  // the pressure.
  // Otherwise, translateModel("IDEAS.Fluid.FMI.ExportContainers.Examples.FMUs.ResistanceVolume")
...
as required from medium model \"IDEAS.Media.Air\".");
    assert(noEvent(T <= 423.15), "In "   + getInstanceName() + ": Temperature T exceeded its maximum allowed value of 150 degC (423.15 Kelvin)
as required from medium model \"IDEAS.Media.Air\".");

  assert(noEvent(p >= 0.0), "Pressure (= " + String(p) + " Pa) of medium \"IDEAS.Media.Air\" is negative\n(Temperature = " + String(T) + " K)");
