...
package Water "Package with model for liquid water with constant density"
  ...
  String(T_min-273.15) + " degC (" + String(T_min) + " Kelvin) as required from medium model \"IDEAS.Media.Water\".");

    assert(noEvent(T <= T_max), "In " + getInstanceName() + ": Temperature T = " + String(T) + " K exceeded its maximum allowed value of " +
  String(T_max-273.15) + " degC (" + String(T_max) + " Kelvin) as required from medium model \"IDEAS.Media.Water\".");

    assert(noEvent(p >= 0.0), "Pressure (= " + String(p) + " Pa) of medium \"IDEAS.Media.Water\" is negative\n(Temperature = " + String(T) + " K)");
