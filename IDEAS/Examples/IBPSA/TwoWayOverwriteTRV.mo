...
model TwoWayOverwriteTRV
  ...
  Modelica.Blocks.Sources.RealExpression yExp(y=
    ...
    annotation (Placement(transformation(extent={{-94,30},{-54,50}})));
  ...
  IBPSA.Utilities.IO.SignalExchange.Overwrite oveVal(description=
      ...
      min=0,
      max=1,
      unit="1")) "Overwrite valve actuator signal"
...
equation
  ...
  connect(yExp.y, oveVal.u)
    annotation (Line(points={{-52,40},{-40,40}}, color={0,0,127}));
  connect(oveVal.y, val.y)
