...
<li>Some old models have been removed, and the library has been cleaned up in general.</li>
<li>New example models and unit tests have been added that better demonstrate how IDEAS can be used. In general, the documentation of the library has improved.</li>
<li>The OuterWall model now natively supports shading from external buildings using the BuildingShade model.</li>
<li>The InternalWall model now has the option to simulate natural convection through open doors or cavities.</li>
<li>The license has been modified from MSL2 to BSD3.</li>
<li>LIDEAS has been merged into IDEAS. LIDEAS contains code for linearising an IDEAS building envelope model and for exporting it as a state space model. For more details, see http://www.ep.liu.se/ecp/118/005/ecp1511851.pdf</li>
</ul>
<p>Detailed release notes:</p>
<ul>
<li>The RectangularZoneTemplate has been extended considerably. Some changes are not reverse compatible.<br>
...
The implementation now has final alpha=0 in the PrescribedHeatFlowRate blocks, which can significantly reduce the size of algebraic loops that are formed.</li>
<li>A bug has been solved in the BuildingShade model that caused it to only work correctly for south-oriented surfaces.</li>
<li>A bug has been solved in the computation of the absolute humidity. The shading models now compute the impact on the diffuse solar irradiation more accurately. Although the used models are relatively simple, they are more accurate than the previous implementation, which disregarded the influence completely.</li>
<li>Several bugs have been solved in the implementation of the Adsolair internal controller, which caused the unit not to work as intended.</li>
<li>A bug has been fixed where the Zone model did not work when the air medium did not contain water vapour.</li>
<li>The zone model now has several options for defining the occupancy internally. A fixed occupancy, a table, or an external input can be used. The relevant block is replaceable such that users can write their own occupancy models.</li>
<li>Fixed the propagation of T_start in MonoLayerDynamic.</li>
<li>A bug has been fixed where the shading control input of the window model was not displayed correctly.</li>
<li>Two new shading models have been added: a horizontal fin shading model and the combination of a horizontal fin shading model with an overhang.</li>
<li>ThreeWayValveMotor now also has a valve leakage since the old implementation can lead to singularities when the flow rate is exactly zero.</li>
...
<li>The window model has been extended such that it can be configured to consist of nWin identical windows. This makes it easier to correctly configure the shading of multiple identical windows. The heat flow rates are simply scaled by nWin.</li>
<li>IDEAS 2.0 is based on IBPSA 3.0, which includes general performance and documentation improvements. Notable additions from the IBPSA library are:<br>
...
A model has been added for directly writing .csv result files. These result files can also be used as an input for a CombiTimeTable.</li>
<li>IDEAS now supports multiple trace substances for the zone model. I.e. other trace substances than CO2 are also supported.</li>
<li>The Zone models now have the option for computing thermal heat gains due to lighting. Various zone types are supported, which determine the illuminance requirements. The lighting type determines the electrical power required for this illuminance. A lighting controller determines whether the lighting is enabled. </li>
</ul>
</html>"));
end Version_2_0_0;
