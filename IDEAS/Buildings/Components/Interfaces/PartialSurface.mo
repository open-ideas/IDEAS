...
partial model PartialSurface "Partial model for building envelope component"
  ...
  parameter Real hVertical=if IDEAS.Utilities.Math.Functions.isAngle(inc,Modelica.Constants.pi,0.01) or IDEAS.Utilities.Math.Functions.isAngle(inc,0,0.01) then 0 else hzone_a "Vertical surface height, height of the surface projected to the vertical, 0 for floors and ceilings" annotation(Evaluate=true);
  parameter Real hRef_a= if IDEAS.Utilities.Math.Functions.isAngle(inc,0,0.01) then hzone_a else 0  "Height above the zone floor at propsbus_a. Height where the surface starts. e.g. 0 for walls at floor level and floors.  "
                                                                                                                                                                                                        annotation(Evaluate=true);
